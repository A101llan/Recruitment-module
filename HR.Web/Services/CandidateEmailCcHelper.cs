...

    {
...

        {
...

            {
                ...
                var panelistIds = selectedPanelistUserIds ?? Enumerable.Empty<int>();

                if (!panelistIds.Any(id => id > 0))
...

            {
                ...
                var hrContactIds = selectedHrContactIds ?? Enumerable.Empty<int>();

                if (!hrContactIds.Any(id => id > 0))
...

        {
            ...
            if (string.Equals(role, "Panelist", StringComparison.OrdinalIgnoreCase))
            ...
            return string.Equals(role, "Admin", StringComparison.OrdinalIgnoreCase) && user.IsPanelist;
...

        {
            ...
            var primaryEmail = primaryRecipientEmail.Trim();

            emails.RemoveAll(e => string.Equals(e, primaryEmail, StringComparison.OrdinalIgnoreCase));
