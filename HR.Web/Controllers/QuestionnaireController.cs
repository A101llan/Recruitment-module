...
namespace HR.Web.Controllers
{
    ...
    public class QuestionnaireController : Controller
    {
        ...
        /// <summary>
        /// Test questionnaire interface
        /// </summary>
        public ActionResult Test(int positionId)
        {
            var position = _uow.Positions.Get(positionId);
            if (position == null) return HttpNotFound();
            ...
            var viewModel = new QuestionnaireTestViewModel
            {
                Position = position,
                Questions = positionQuestions.Select(pq => new QuestionnaireTestItem
                {
                    QuestionId = pq.Question.Id,
                    QuestionText = pq.Question.Text,
                    QuestionType = pq.Question.Type,
        ...
        public ActionResult SubmitTest(QuestionnaireTestViewModel model)
        {
            try
            {
                // Calculate score for test answers
                var score = 0m;
                var maxScore = 0m;
                ...
                {
                    var questionEntity = _uow.Questions.Get(question.QuestionId);
                    var questionScore = _scoringService.CalculateQuestionScore(
                        questionEntity, question.Answer, model.Position.Id);
                    var questionMaxScore = _scoringService.GetMaxScoreForQuestion(
                        questionEntity, model.Position.Id);

                    score += questionScore;
                    maxScore += questionMaxScore;

                    results.Add(new TestQuestionResult
                    {
                        QuestionText = question.QuestionText,
                        Answer = question.Answer,
                        Score = questionScore,
                        MaxScore = questionMaxScore,
                        Percentage = questionMaxScore > 0 ? (questionScore / questionMaxScore) * 100 : 0
                    });
                }

                var testResult = new QuestionnaireTestResult
                {
            ...
            {
                ModelState.AddModelError("", "Error processing test: " + ex.Message);
